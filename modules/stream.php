<?php
		...
		switch ($encoder) {
			...
			case 'ffmpeg':
				...
				$stream_cmd = '"'.$ffmpeg_path.'"'.
					...
					' '.$subtitle_ffmpeg_cmd.
					// その他
					' -flags +loop+global_header -movflags +faststart -threads auto -max_interleave_delta 500K'.
					// 出力
					' stream'.$stream.'.m3u8';
			...
			case 'QSVEncC':
				...
				$stream_cmd = '"'.$qsvencc_path.'"'.
					...
					' '.$subtitle_other_cmd.
					// その他
					' --avsync forcecfr --fallback-rc --max-procfps 90 --output-thread 0 -m max_interleave_delta:500K'.
					// 出力
			...
			case 'NVEncC':
				...
				$stream_cmd = '"'.$nvencc_path.'"'.
					...
					' '.$subtitle_other_cmd.
					// その他
					' --avsync forcecfr --max-procfps 90 --output-thread 0 -m max_interleave_delta:500K'.
					// 出力
					' -o stream'.$stream.'.m3u8';
			...
			case 'VCEEncC':
				...
				$stream_cmd = '"'.$vceencc_path.'"'.
					...
					' '.$subtitle_other_cmd.
					// その他
					' --avsync forcecfr --max-procfps 90 -m max_interleave_delta:500K'.
					// 出力
					' -o stream'.$stream.'.m3u8';
	...
	function stream_stop($stream, $allstop = false, $exclude_tstask = false) {
		...
		global $inifile, $udp_port, $process_csv, $arib_subtitle_timedmetadater_exe, $ffmpeg_exe, $qsvencc_exe, $nvencc_exe, $vceencc_exe, $tstask_exe, $tstaskcentreex_path, $segment_folder, $TSTask_shutdown;
		...
		// このストリームを終了する
			...
			foreach ($process as $value) {
				...
				// arib-subtitle-timedmetadater
				if (strpos($value['CommandLine'], $arib_subtitle_timedmetadater_exe) !== false and @strpos($value['CommandLine'], strval($stream_port)) !== false) {
