...
- **基本**
  ...
  - basic認証設定（要設定）
  - ストリーム起動の監視
  - ブラウザからの動作設定（個人設定のみ実装・環境設定は未実装）
  - Chromecast 等 Google Cast 対応デバイスへのキャスト（暫定実装）
    - 録画ファイルをテレビの大画面で再生させたい時などに使えると思います
    - CastV2inPHP というライブラリを使用しています（ありがとうございます）
    - 予め Bonjour という iTunes についてくるソフトを TVRemotePlus にインストールしておいてください
      - Chromecast のスキャンに利用します
  - 複数ストリームの同時配信（予定）
  - 録画ファイルリストの自動更新（予定）
...
5. 下の「 TwitterAPI 開発者アカウントの取得について」の項目を参考にし、Twitter API アカウントを取得し、アプリケーションを作成します。
   - 既に Twitter API アカウントを持っている方、ツイート投稿機能を利用しない方は適宜ステップを飛ばして下さい
その後、config.php を上の手順で開き、作成したアプリケーションのコンシューマーキー等を入力して下さい。
6. C:\（TVRemotePlusのあるフォルダ）\bin\TSTask\BonDriver\ フォルダに、いつも TVTest 等で使用している BonDriver と、.ch2 ファイル（チャンネル設定ファイル）を入れてください。
   - BonDriver は 32bit・64bit 両方対応しています、インストーラーでどちらかを指定してください
   - B-CASカードを内蔵カードリーダーで利用している場合は、WinsCard.dll を TSTask.exe と同じフォルダに入れてください
7. 設定を終えたら、**デスクトップに追加された赤い羽根のショートカットをダブルクリックし、Web サーバーを起動します。**
...
9. 後は適当に使ってください。
   - 比較的頻繁にアップデートします、できるだけ最新の TVRemotePlus を使うことをお勧めします
     - config.php は新機能が追加された場合は変更される事があるため、アップデートでインストールすると動かない場合があります
...
![Screenshot](docs/screenshot4.png)
 ...
 - 右上のボタンを押すと、サブメニューが開きます。ChromeCast でのキャストや録画ファイルリストの手動更新などに利用します。
   ...
   - リストの更新には時間がかかる事があります（特にサムネイルの生成に時間がかかります・1000 以上録画ファイルがある場合、数十分～数時間は待ったほうが良いと思います…）
...
- ffmpeg（ v4.2・UDP 受信 → エンコードに利用）
- QSVEncC（ v3.24・UDP 受信 → ハードウェアエンコードに利用）
- NVEncC（ v4.45・UDP 受信 → ハードウェアエンコードに利用）
...
また、このソフトを利用して起こったいかなる不利益も、私は一切の責任を負いかねます。あくまで自己責任にて利用してください。  
...
ch_sid.txt は NicoJK にて使われていたものをチャンネル名を現在のものに修正した上で使わせて頂いています。  
