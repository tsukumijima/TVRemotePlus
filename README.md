...
（実装予定）とあるものは現在実装出来ていないものの、今後実装する予定のある機能です。  
 ...
 - **Basic 認証も（一応）できます**
   - おまけ機能で作者も使っていないので、動くかどうかは正直かなり微妙…
     - 一部の機能が動かないこともあるかもしれません
   - セキュリティ的な面では弱いので、気になる方は適宜対策を取ってください（丸投げ）
   - デフォルトではオフになっています、利用する場合は設定からオンにしてください
   - デフォルトのユーザー名は user 、パスワードは 12345678 になっていますが、Basic 認証機能を使う場合は必ず変えるようにしてください
 - **リバースプロキシを使い外部から視聴できるようにする事もできます（上級者向け・[詳しくはこちら](docs/Reverse_Proxy.md)）**
   - 上級者向けですが、Apache や Nginx などの Web サーバーに用意されているリバースプロキシを使えば、外部から https://example.com/tvrp/ のような URL で TVRemotePlus アクセス出来るようにすることもできます
   - 基本的にはセキュリティの面でも VPN 経由でアクセスする事をおすすめしますが、この方法であれば速度低下や手間がかかるといった VPN のデメリットがなくなります
     ...
     - この他、Basic 認証をオンにするなど、各自でセキュリティ対策を行ってください
       - 現状、TVRemotePlus には外部から不正なリクエストを受けた場合に PHP の関数全て（ OS コマンドも含む）を実行できる可能性のある脆弱性があります
         - Basic 認証をかけるなどして外部からの不正なリクエストを断つか、ローカル内で使っていれば問題はありません
         - 近いうちに修正を予定していますが、システムの根幹を弄ることとなるため後回しになるかもしれません…
     - 基本的なリバースプロキシのセットアップ方法はここでは割愛します（全部書いてたら終わらない…）
 - **キーボードショートカット（ほぼ隠し機能）**
   - 特記のあるものを除き基本的に入力フォーム以外にフォーカスした状態で行ってください
   - クリップボード内の画像の貼り付け
     - ツイート入力フォームにフォーカスした状態で Ctrl + V を押すとできます
     - あまり意味のない隠し機能です
   - ツイート入力フォームへフォーカスする
     - Tab キーでフォーカスできます
     - これも隠し機能です
...
### トラブルシューティング　
 ...
 - **インストールフォルダや録画ファイルのあるフォルダへのパスに日本語(全角文字)が含まれていると、正常に動きません。**
   ...
   - 例えば、 C:\テレビ\TVRemotePlus\ だとか C:\freesoft\ＴＶＲＰ\ はエラーになる事があります。
...
[Twitter](https://twitter.com/TVRemotePlus) か [5ch のロケフリスレ](http://mevius.5ch.net/test/read.cgi/avi/1569733081) などで不具合報告やフィードバックは受け付けておきます。
