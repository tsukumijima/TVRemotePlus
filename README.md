...
6. C:\（TVRemotePlusのあるフォルダ）\bin\TSTask\BonDriver\ に、いつも TVTest 等で使用している BonDriver と、.ch2 ファイル（チャンネル設定ファイル）を入れてください（ TSTask は 64bit 版です、**TVTest（32bit） を利用している場合は 64bit 版 BonDriver を調達してください**（ 32bit 版の BonDriver は 64bit ソフトでは利用できないためです））。
