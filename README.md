...
3. **下の「 TwitterAPI 開発者アカウントの取得について」の項目を参考にし、Twitter API 開発者アカウントを取得し、アプリケーションを作成する（任意）**
...
### トラブルシューティング　
 ...
 - **ストリームが正常に終了できない（チャンネルが変わらない）場合は、環境設定 → ［ストリーム終了時に TSTask を強制終了する］をオンにしてみてください**
   - この設定をオンにすると TSTask を taskkill /F で強制終了します
   - 一部の環境では TSTask が taskkill だけでは終了できないらしく、そのための設定です
   - また、BonDriverProxyEx や Spinel を使うと改善した、という報告もあります
