...
# <img alt="Logo" src="logo.png" width="40%">
...
6. C:\（TVRemotePlusのあるフォルダ）\bin\TSTask\BonDriver\ に、いつも TVTest 等で使用している BonDriver を入れてください（ TSTask は 64bit 版です、**TVTest（32bit） を利用している場合は 64bit 版 BonDriver を調達してください**（ 32bit 版の BonDriver は 64bit ソフトでは利用できないためです））。
...
8. ウインドウがすぐに閉じてしまう場合、何らかのエラーで Web サーバーが起動できていません。コマンドプロンプトから　C:\（TVRemotePlusのあるフォルダ）\bin\Apache\bin\httpd.exe を D&D で実行し、エラーログを確認してください。
9. ブラウザの URL 欄に **http://( TVRemotePlus の動いている PC のローカル IP アドレス):8000/ と入力し、TVRemotePlus の Web アプリへアクセスすると、使えるようになっているはずです。**
10. 後は適当に使ってください。（もしかするとエラーが出てうまく動かない場合があるかもしれませんが、ほとんどの場合、設定ミスによるものだと思います…）
## PWAについて
...
### インストール手順
 ...
 - 何故かスプラッシュ画面のアイコンが小さい場合がありますが、原因は不明です（ Chrome の自動推定という話らしいですが…）
