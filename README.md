 ...
 - **放送中のテレビが見れます**
   ...
   - **ピクチャーインピクチャーにも対応しています**
     ...
     - Android 8.0 以降で最新版の Chrome を使っているのにプレイヤーにピクチャーインピクチャーボタンが出てこない場合は、アドレスバーに chrome://flags と入力し、  
       出てきた検索窓から Picture-in-Picture と検索すると Picture-in-Picture Web API For Android という項目があるので、Default となっているところを Enabled にし、Chrome を再起動するとボタンが表示されるはずです（テスト中の機能のようなので場合によっては存在しないかも）
   - **プレイヤーをフルスクリーンにする事ができます**
     ...
     - iOS 端末では Safari の仕様でフルスクリーンにするとブラウザデフォルトのプレイヤーが使用されます
       - そのため、コメントを表示させたい場合はブラウザフルスクリーンを使うようにしてください
       - iPadOS ( iOS13 以降の iPad ) ではフルスクリーンがサポートされているため、フルスクリーンにしても通常のプレイヤーで再生できるようになっています
...
2. **解凍したフォルダの中の install.bat を実行し、インストーラー通りに進める**
   ...
   - C:\Program Files や C:\Users 以下へのインストールも推奨しません（個人的には `C:\DTV\TVRemotePlus` とかがおすすめ）
     - 特に C:\Program Files 以下は半角スペースが入っている上に権限が特殊なので最悪です（動きません）
...
- arib-subtitle-timedmetadater (4.0.4・ARIB 字幕を ID3 メタデータに変換)
