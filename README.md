...
- **基本**
  ...
  - Chromecast 等 Google Cast 対応デバイスへのキャスト（暫定実装）
    ...
    - やろうと思えば出来るとは思いますが、現状コメントや字幕の再生には対応していません…
      - 手元のスマホにはコメントが流れるので、スマホのコメントを見ながらテレビで録画を見るのもありかもしれません
    - v1.2.0 から、PC か Android の Chrome でかつ https でアクセスした場合に Chrome ネイティブのキャスト機能を使うようにしました
      - Chromecast デバイスがうまくスキャンできない環境などでもキャスト出来るようになります
      - 基本的に機能としては上位互換なので、Chrome を使っている場合は https でアクセスした上でキャストしてみてください
    - 以前のキャスト機能は非対応ブラウザ向けとして残しています
      - CastV2inPHP というライブラリを使用しています（ありがとうございます）
      - 予め Bonjour という iTunes についてくるソフトを TVRemotePlus にインストールし、その後再起動しておいてください
        - Chromecast のスキャンに利用します
      - Windows10 1809 以前では、デバイスがスキャン出来ない不具合があるようです
...
- NVEncC（ v4.50・UDP 受信 → ハードウェアエンコードに利用）
