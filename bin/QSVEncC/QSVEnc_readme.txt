...
Win 7/8には.NET Framework 4.5はインストール済みなので、問題無いですね。
...
をインストールし、
includeファイルのディレクトリに"<WinDDK>\inc\atl71"を、
win32用ライブラリファイルのディレクトリに"<WinDDK>\lib\ATL\i386"を、
x64用ライブラリファイルのディレクトリに"<WinDDK>\lib\ATL\amd64"を
追加することで解決できます。

"avisynth_c.h"がないといわれる場合、
qsv_version.hの
#define ENABLE_AVISYNTH_READER 1
を
#define ENABLE_AVISYNTH_READER 0
に変更してAvsReaderを無効化するか、
Avisynth 2.5.8 / 2.6.0をSDK付きでインストールし、
includeファイルのディレクトリに"<Avisynth>\FilterSDK\include"を
追加することで解決できます。
本ビルドはAvisynth2.5.8の"avisynth_c.h"を使用しビルドしています。

"vapoursynth.h"がないといわれる場合、
qsv_version.hの
#define ENABLE_VAPOURSYNTH_READER 1
を
#define ENABLE_VAPOURSYNTH_READER 0
に変更してVSReaderを無効化するか、
VapourSynthをインストールし、
includeファイルのディレクトリに"<VapourSynth>\sdk\include"を
追加することで解決できます。
...
に変更してOpenCLによるGPU情報の取得を無効化するか、
OpenCL SDKをインストールし、
includeファイルのディレクトリにOpenCL SDK へのパスを
追加することで解決できます。
