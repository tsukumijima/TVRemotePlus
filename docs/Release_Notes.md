  ...
  - **v2.0.0** -2020/01/**
    ...
    - **ストリームの複数同時配信に対応した**
      ...
      - **複数同時配信への対応にともない、ストリーム開始設定からどのストリームでストリームを開始するか選択できるようにした**
        ...
        - 何も指定がない場合、ライブ放送の場合は現在視聴しているストリームで、ファイル再生の場合は空いているストリームでそれぞれストリームを開始します（予定）
        - デフォルトは Stream 1・2・3・4 の4ストリームが用意されており、4ストリーム全て使い切っているときのみ Stream 5 以降が利用可能になります（予定）
