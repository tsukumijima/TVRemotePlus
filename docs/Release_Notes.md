...
# Release Notes
  ...
  - **v2.7.0** - 2022/04/00
    - **リリース概要**
      ...
      - 本当は v2.6.1 で開発終了のつもりだったのですが、BS新局のニコニコ実況対応を入れる必要が出てきたため、ついでに積年の不具合をいろいろ修正しました。
        - ついでとは言いつつ、結局前々から顕在化していた様々な不具合の修正がメインになってしまっています…（2週間が丸々溶けた）。
      ...
      - **KonomiTV 向けに大幅に改良された DPlayer をバックポート**
        - [DPlayer](https://github.com/tsukumijima/DPlayer) は JavaScript 製の動画プレイヤーです。TVRemotePlus 向けに大幅に不具合修正や改善を行ったものを、KonomiTV でもさらに改良して利用しています。
      ...
      - **tsreadex を導入し、エンコードがより安定するように改善**
        - [tsreadex](https://github.com/xtne6f/tsreadex) は ffmpeg などのエンコーダーでうまく認識できない放送波の TS ストリームを、エンコーダーが認識しやすい形に整形するためのツールです。おもに KonomiTV 向けに開発していただいたものです。
      ...
      - **画面のレイアウトを視聴画面の︙メニューから1クリックで変更できるように改善**
        ...
        - メニューはPC・タブレットでのみ表示されます。
      - **録画番組のサムネイル生成ロジックを大幅に改善**
        - 壊れた TS ファイルなど何らかの理由でサムネイル生成ができない場合がありますが、その際にリスト更新処理ごとフリーズしてなかなかリスト更新が進まない、といったことがなくなりました。
          - 具体的には、ffmpeg でのサムネイル生成コマンドの実行時に、タイムアウトを設定するようにしました。
          - タイムアウトを設定するがために外部ライブラリを導入したり、かなり大掛かりになって大変だった…。
